...
class CBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.5, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.conv1 = nn.Conv1d(3, 3, 1)
        self.conv2 = nn.Conv1d(3, 3, 1)
        self.attn = nn.Conv1d(3, 3, 1)#, padding=0, groups=1)
#        self.attn = nn.Conv2d(dim, dim, 13, padding=6, groups=dim)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = nn.LayerNorm(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = CMlp(in_features=3, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

    def forward(self, x, mask=None):
        if mask is not None:
            x = x + self.drop_path(self.conv1(self.attn(mask * self.conv1(self.norm1(x)))))
        else:
            # x = x + self.drop_path(self.conv1(self.attn(self.conv1(self.norm1(x)))))
            # 提取每个操作
            normed_x = self.norm1(x)
            conv1_out = self.conv1(normed_x)
            attn_out = self.attn(conv1_out)
            conv2_out = self.conv2(attn_out)
            dropout_out = self.drop_path(conv2_out)
