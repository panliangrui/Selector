...
class MaskedImageInpaintingTransformer(nn.Module):
    def __init__(
            self,n_layer=2,  # number of layers in transformer
            content_seq_len=512,  # length of content sequences
            embd_pdrop=0.,  # embedding dropout prob
            n_embd=512,  # the embed dim
            n_head=8,  # the number of heads
            attn_pdrop=0.1,  # attention dropout prob
            resid_pdrop=0.1,  # residual attention dropout prob
            block_activate='GELU',
    ...
    def forward(self,batch,return_loss=False,return_logits=True,return_att_weight=False,**kwargs):
...
# model = MaskedImageInpaintingTransformer()
# print(model)