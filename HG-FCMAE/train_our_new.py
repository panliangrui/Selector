...
from models.GraphMAE import fusion_model_GraphMAE
from models.HGMAE import PreModel
from models.our_new import fusion_model_our
...
def main(args):
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            if fusion_model == 'fusion_model_mae_2':
                ...
                model = fusion_model_DMAE(in_feats=1024,
                                          n_hidden=args.n_hidden,
                                          out_classes=args.out_classes,
                                          dropout=drop_out_ratio,
                                          train_type_num=len(args.train_use_type)).to(device)

            elif fusion_model == 'fusion_model_our':
            ...
            with torch.no_grad():
                for id in test_data:
                    data = all_data[id]
                    data.to(device)
                    (one_x, multi_x), fea, (att_1, att_2), _ = t_model(data, args.train_use_type, args.train_use_type,
                                                                       mix=args.mix)
            ...
            torch.save(t_model.state_dict(),'../results_ab/ARMAConv/LUAD/' + sys_time.strftime('%Y-%m-%d') + label + '_' + str(seed) + '_' + str(n_fold) + '.pth')
    ...
    joblib.dump(all_gnn_time, '../results_ab/ARMAConv/LUAD/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results_ab/ARMAConv/LUAD/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
...
def get_params():
    ...
    parser.add_argument("--fusion_model", type=str, default="fusion_model_our", help="")
    ...
    parser.add_argument("--epochs", type=int, default=20, help="Cycle times of model training")
    ...
    parser.add_argument("--leave_unchanged", type=float, default=0.0, help="The ratio of nodes left unchanged (no mask), but is asked to reconstruct.")
