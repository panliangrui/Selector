...
def main(args):
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            torch.save(t_model.state_dict(),'../results/LIHC/Convnextv2/' + sys_time.strftime('%Y-%m-%d') + label + '_' + str(seed) + '_' + str(n_fold) + '.pth')
    ...
    joblib.dump(all_gnn_time, '../results/LIHC/Convnextv2/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results/LIHC/Convnextv2/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
...
def get_params():
    ...
    parser.add_argument("--fusion_model", type=str, default="fusion_model_convnextv2", help="")
    ...
    parser.add_argument("--batch_size", type=int, default=200, help="Data volume of model training once")
