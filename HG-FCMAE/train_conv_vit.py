...
def main(args):
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            if fusion_model == 'fusion_model_mae_2':
                ...
                model = fusion_model_MaskedAutoencoderConvViT(args, in_feats=1024,
                                                              n_hidden=args.n_hidden,
                                                              out_classes=args.out_classes,
                                                              dropout=drop_out_ratio,
                                                              train_type_num=len(args.train_use_type)).to(device)
            ...
            torch.save(t_model.state_dict(), '../results/KIRC/ConvMAE/' + sys_time.strftime('%Y-%m-%d') + label + '_' + str(seed) + '_' + str(
                           n_fold) + '.pth')
    ...
    joblib.dump(all_gnn_time, '../results/KIRC/ConvMAE/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results/KIRC/ConvMAE/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
...
def get_params():
    ...
    parser.add_argument("--fusion_model", type=str, default="fusion_model_MaskedAutoencoderConvViT", help="")
