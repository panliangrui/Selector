...
def main(args): 
    ...
    label = "{}_{}_lr_{}_{}_coxloss".format(cancer_type, details, lr,format_of_coxloss)
    ...
    for seed in range(start_seed,start_seed+repeat_num):
        ...
        for train_index, test_index in kf.split(patients,kf_label):
            ...
            # torch.save(t_model.state_dict(), 'J:\\pytorch projects\\HGCN-main\\results\\luad\\mae\\'+sys_time.strftime('%Y_%m_%d')+label+'_'+str(seed)+'_'+str(n_fold)+'.pth')
    ...
    # joblib.dump(all_gnn_time,'J:\\pytorch projects\\HGCN-main\\results\\luad\\mae\\'+sys_time.strftime('%Y-%m-%d-%H-%M')+label+'.pkl')
    # joblib.dump(all_each_model_time,'J:\\pytorch projects\\HGCN-main\\results\\luad\\mae\\'+sys_time.strftime('%Y-%m-%d-%H-%M')+label+'.pkl')
...
def get_params():
    ...
    parser.add_argument("--epochs", type=int, default=12, help="Cycle times of model training")
