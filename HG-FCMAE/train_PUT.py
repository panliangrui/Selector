...
def main(args):
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            torch.save(t_model.state_dict(),'../results/LUSC/PUT/' + sys_time.strftime('%Y-%m-%d') + label + '_' + str(seed) + '_' + str(n_fold) + '.pth')
    ...
    joblib.dump(all_gnn_time, '../results/LUSC/PUT/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results/LUSC/PUT/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')


def get_params():
    parser = argparse.ArgumentParser()
    parser.add_argument("--cancer_type", type=str, default="lusc", help="Cancer type")
    ...
    parser.add_argument("--fusion_model", type=str, default="fusion_model_PUT", help="")
