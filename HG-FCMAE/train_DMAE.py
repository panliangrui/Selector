...
from models.covmae_new import fusion_model_MaskedAutoencoderConvViT
# from models.convnextv2_new import fusion_model_convnextv2
from models.convnextv2 import fusion_model_convnextv2
from models.DMAE import fusion_model_DMAE
...
def prediction(all_data, v_model, val_id, patient_and_time, patient_sur_type, args):
    ...
    with torch.no_grad():
        for i_batch, id in enumerate(val_id):
            ...
            _, out_pre, out_fea, out_att, _ = v_model(graph, args.train_use_type, use_type_eopch, mix=args.mix)
            lbl_pred = out_pre[0]
    ...
    for i_batch, id in enumerate(train_data):
        ...
        if len(args.train_use_type) == 1:
            ...
            if args.train_use_type[0] in all_data[id].data_type:
                graph = all_data[id].to(device)
                out_pre, out_fea, out_att, fea_dict = model(graph, args.train_use_type, args.train_use_type,
                                                            mix=args.mix)
                lbl_pred = out_pre[0]
                use_type_eopch = args.train_use_type
                num_of_model = 1
            ...
            graph = all_data[id].to(device)
            loss_distillation_embedding, out_pre, out_fea, out_att, fea_dict = model(graph, use_type_eopch, use_type_eopch, mask, mix=args.mix)
            lbl_pred = out_pre[0]
        ...
        if len(args.train_use_type) == 1 and args.train_use_type[0] not in all_data[id].data_type:
            ...
            if args.add_mse_loss_of_mae:
                ...
                mse_loss_of_mae += loss_distillation_embedding
...
def main(args):
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            if fusion_model == 'fusion_model_mae_2':
                ...
                print(model)
            elif fusion_model == 'fusion_model_MaskedAutoencoderConvViT':
                model = fusion_model_MaskedAutoencoderConvViT(in_feats=1024,
                                                              n_hidden=args.n_hidden,
                                                              out_classes=args.out_classes,
                                                              dropout=drop_out_ratio,
                                                              train_type_num=len(args.train_use_type)).to(device)
            elif fusion_model == 'fusion_model_convnextv2':
                model = fusion_model_convnextv2(in_feats=1024,
                                              n_hidden=args.n_hidden,
                                              out_classes=args.out_classes,
                                              dropout=drop_out_ratio,
                                              train_type_num=len(args.train_use_type)).to(device)
            elif fusion_model == 'fusion_model_DMAE':
                model = fusion_model_DMAE(args, in_feats=1024,
                                          n_hidden=args.n_hidden,
                                          out_classes=args.out_classes,
                                          dropout=drop_out_ratio,
                                          train_type_num=len(args.train_use_type)).to(device)
            ...
            for epoch in range(epochs):
                ...
                all_loss, t_train_ci, t_train_ci_img, t_train_ci_rna, t_train_ci_cli = train_a_epoch(model, train_data,
                                                                                                     all_data,
                                                                                                     patient_and_time,
                                                                                                     patient_sur_type,
                                                                                                     batch_size,
                ...
                t_test_loss, test_ci, test_img_ci, test_rna_ci, test_cli_ci = prediction(all_data, model, test_data,
                                                                                         patient_and_time,
                                                                                         patient_sur_type, args)
                v_loss, val_ci, val_img_ci, val_rna_ci, val_cli_ci = prediction(all_data, model, val_data,
            ...
            with torch.no_grad():
                for id in test_data:
                    data = all_data[id]
                    data.to(device)
                    _, (one_x, multi_x), fea, (att_1, att_2), _ = t_model(data, args.train_use_type, args.train_use_type,
                                                                       mix=args.mix)
                    ...
                    for i, type_name in enumerate(['img', 'rna', 'cli']):
                        if type_name in data.data_type:
                            _, (one_, _), one_fea, (_, _), _ = t_model(data, args.train_use_type, use_type=[type_name],
                                                                    mix=args.mix)
                    ...
                    for i, two_type_name in enumerate([['img', 'rna'], ['img', 'cli'], ['rna', 'cli']]):
                        _, (one_, two_), one_fea, (_, _), _ = t_model(data, args.train_use_type, use_type=two_type_name,
                                                                   mix=args.mix)
            ...
            torch.save(t_model.state_dict(),'../results/UCEC/DMAE/' + sys_time.strftime('%Y-%m-%d') + label + '_' + str(seed) + '_' + str(
                           n_fold) + '.pth')
    ...
    joblib.dump(all_gnn_time, '../results/UCEC/DMAE/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results/UCEC/DMAE/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')


def get_params():
    parser = argparse.ArgumentParser()
    parser.add_argument("--cancer_type", type=str, default="ucec", help="Cancer type")
    ...
    parser.add_argument("--fusion_model", type=str, default="fusion_model_DMAE", help="")
    ...
    parser.add_argument("--feat_drop", type=float, default=.2, help="input feature dropout")
    ...
    parser.add_argument("--feat_mask_rate", type=str, default="0.5,0.005,0.8", help="""The mask rate. If provide a float like '0.5', mask rate is static over the training. 
    ...
    parser.add_argument("--mp_edge_mask_rate", type=str, default="0.5,0.005,0.8", help="""The mask rate. If provide a float like '0.5', mask rate is static over the training. 
                                    If provide two number connected by '~' like '0.4~0.9', mask rate is uniformly sampled over the training.
                                    If Provide '0.7,-0.1,0.5', mask rate starts from 0.7, ends at 0.5 and reduce 0.1 for each epoch.""")
